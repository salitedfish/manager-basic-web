<template>
    <div class="app-container">
        ...
        <select-role ref="selectRef" :postId="queryParams.postId" @ok="handleaddPostRole" />
    </div>
</template>
...
import { listPostRole, delPostRole, addPostRole } from '@/api/system/post';
...
function getList() {
    loading.value = true;
    listPostRole(queryParams).then((response: any) => {
...
//添加角色
async function handleaddPostRole(roleIds: any) {
    const params={
        roleIds:roleIds,
        postId:route.query.Id,
    }
    await addPostRole(params)
    queryParams.pageNum = 1;
    getList();
}
...
function cancelAuthUser(row: any) {
    proxy!.$modal
        ...
        .then(function () {
            return delPostRole({ roleIds: [row.roleId], postId: queryParams.postId });
        })
        .then(() => {
            getList();
...
function cancelAuthUserAll(row: any) {
    ...
    proxy!.$modal
        ...
        .then(function () {
            return delPostRole({ postId: postId, roleIds: uIds });
        })
        .then(() => {
            getList();
            proxy!.$modal.msgSuccess('取消授权成功');
