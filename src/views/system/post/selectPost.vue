<template>
    <!-- 授权岗位 -->
    <el-dialog v-model="visible" title="选择岗位" width="800px" top="5vh" append-to-body>
...
function handleSelectionChange(selection: any[]) {
    postIds.value = selection.map(item => item.postId);
}
// 查询表数据
function getList() {
    listPost(queryParams).then((res: any) => {
        List.value = res.rows;
...
function handleSelectUser() {
    if (postIds.value.length == 0) {
        proxy!.$modal.msgError('请选择要分配的岗位');
        return;
    }
    visible.value = false;
